...
If you become aware of any violation of this policy, please report it to abuse@certsync.com.`
    ...
    return policies[policySlug as keyof typeof policies] || '# Policy Not Found\n\nThe requested policy could not be found.';
  };
            ...
            >
              <div className="prose prose-lg max-w-none bg-white rounded-lg p-8 shadow-sm border border-neutral-200">
                <ReactMarkdown
                  ...
                  components={{
                    h1: ({ children }) => (
                      <h1 id={children?.toString().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')} className="text-2xl font-bold text-neutral-900 mb-6 pt-4 first:pt-0">
                        {children}
                      </h1>
                    ),
                    h2: ({ children }) => (
                      <h2 id={children?.toString().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')} className="text-xl font-semibold text-neutral-900 mb-4 mt-8 pt-4">
                        {children}
                      </h2>
                    ),
                    h3: ({ children }) => (
                      <h3 id={children?.toString().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')} className="text-lg font-semibold text-neutral-900 mb-3 mt-6">
                        {children}
                      </h3>
                    ),
                    p: ({ children }) => (
